...
const continueButtonTextMap: Partial<Record<CheckoutStep, JSX.Element>> = {
    [CheckoutStep.Address]: <FormattedMessage {...checkoutMessages.addressNextActionName} />,
    [CheckoutStep.Shipping]: <FormattedMessage {...checkoutMessages.shippingNextActionName} />,
    [CheckoutStep.Payment]: <FormattedMessage {...checkoutMessages.paymentNextActionName} />,
    [CheckoutStep.Review]: <FormattedMessage {...checkoutMessages.reviewNextActionName} />,
};

export const getContinueButtonText = (step: CheckoutStep) => continueButtonTextMap[step];
...
export const getAvailableSteps = (items: IItems | undefined) => {
    ...
    const stepsWithViews = CHECKOUT_STEPS.filter(({ withoutOwnView }) => !withoutOwnView);
    return isShippingRequired
        ? stepsWithViews
        : stepsWithViews.filter(({ onlyIfShippingRequired }) => !onlyIfShippingRequired);
};

export const getCurrentStep = (pathname: string, steps: CheckoutStepDefinition[]) => {
...
export const stepSubmitSuccessHandler =
    (push: NextRouter["push"], steps: CheckoutStepDefinition[], activeStepIndex: number) =>
...
export const prepareCartSummaryProducts = (items?: IItems): CartSummaryProps["products"] | undefined =>
