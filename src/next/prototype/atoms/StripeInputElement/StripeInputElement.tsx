...
import { IStripeInputElementProps } from "./types";

/**
 * Wrapper component for Stripe input elements.
 */
const StripeInputElement: React.FC<IStripeInputElementProps> = ({
    onBlur,
    onFocus,
    contentLeft = null,
    contentRight = null,
    ...
    return (
        <S.Wrapper active={active} error={error} ref={elementRef}>
            ...
            <S.InputWrapper>
                {renderStripeElement()}
                {label && (
                    <InputLabel labelBackground={labelBackground} active={active || !!filled}>
                        {label}
                    </InputLabel>
                )}
            </S.InputWrapper>
