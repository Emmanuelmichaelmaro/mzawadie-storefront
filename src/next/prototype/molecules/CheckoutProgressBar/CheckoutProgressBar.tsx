...
import { ICheckoutProgressBarProps } from "./types";
...
const generateLabel = (stepIndex: number, name: string, numberOfSteps: number) => {
    if (stepIndex === 0) {
...
const generateProgressBar = (index: number, currentActive: number, numberOfSteps: number) => {
    if (index !== numberOfSteps - 1) {
        return <S.ProgressBar done={currentActive > index} />;
    }
};

const generateSteps = (steps: ICheckoutStep[], currentActive: number, intl: IntlShape) => {
