...
export const InputSelect: React.FC<IProps> = ({ label, inputProps, ...props }: IProps) => {
    ...
    const customStyles = {
        control: (provided: any, state: { menuIsOpen: any }) => ({
            ...
            background: "none",
            border: state.menuIsOpen ? `1px solid ${secondaryColor}` : `1px solid ${borderColor}`,
            borderRadius: 0,
            boxShadow: 0,
    ...
    const customComponents = {
        Control: (props: ControlProps<any>) => {
            ...
            return (
                <>
                    <components.Control data-test="inputSelect" {...{ customTheme, ...props }} />
                    <InputLabel
    ...
    return <Select customComponents={customComponents} {...props} customStyles={customStyles} />;
};
