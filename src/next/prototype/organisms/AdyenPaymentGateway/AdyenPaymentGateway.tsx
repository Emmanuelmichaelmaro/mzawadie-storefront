...
export interface IProps {
     ...
     */
    submitPaymentSuccess: (order?: CompleteCheckout_checkoutComplete_order | null) => void;
    /**
...
const AdyenPaymentGateway: React.FC<IProps> = ({
    ...
    const adyenClientKey = config?.find(({ field }) => field === "client_key")?.value;
    ...
    const onSubmitAdyenForm = async (state?: AdyenSubmitState, dropin?: AdyenSubmitDropin) => {
        if (!state?.isValid) {
            onError([new Error(intl.formatMessage(adyenErrorMessages.invalidPaymentSubmission))]);
        } else {
    ...
    const onAdyenError = (error?: AdyenError) => {
        if (error?.error) {
            ...
            onError([new Error(intl.formatMessage(adyenErrorMessages.unknownPayment))]);
        }
    };
