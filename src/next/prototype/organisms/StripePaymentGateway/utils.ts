...
export const parsePaymentConfirmationData = (confirmationData: string) => {
    try {
        const paymentAction = JSON.parse(confirmationData) as StripeConfirmationData;
        return {
            paymentAction,
        };
    } catch (parseError) {
...
export const handleConfirmCardPayment = async (
    ...
    try {
        const confirmation = await stripe.confirmCardPayment(paymentAction.client_secret, {
