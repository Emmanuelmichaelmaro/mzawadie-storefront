// @ts-nocheck
import { getValueOrEmpty } from "@mzawadie/core";
...
class PriceRangeFilter extends React.Component<PriceRangeFilterProps, PriceRangeFilterState> {
    ...
    handleClickAway = (event: Event) => {
        if (this.state.active && !this.filterRef.current.contains(event.target as Node)) {
    ...
    render() {
        const Control = (props) => <components.Control {...props} isFocused={this.state.active} />;
