...
export const getBuildManifestPages = (): string[] => {
    ...
    return Object.keys(manifest).filter((url) => !url.includes("/_") && url !== "/404");
};
...
export const getRequestedPageFromURL = (url: string): string | undefined => {
    ...
    return manifest.find((page) => {
        ...
        return manifestPage.every((manifestPathPiece, index) => {
            ...
            return manifestPathPiece === requestedPathPiece || /^\[\w+\]$/.test(manifestPathPiece);
        });
    });
};

export const deleteEntriesForCache = (CACHE_NAME: string) => async (): Promise<void> => {
